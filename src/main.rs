...
fn main() {
    ...
    let each_match = get_each_char(file);

    let total_sum = get_total_sum(each_match);

    println!("The total sum of today is {total_sum}");

    let second_match = get_each_char2();

    let total_sum2 = get_total_sum(second_match);
    ...
    let partly_overlaps = camp_cleanup2(file);
    ...
    let chars = content
        ...
        .fold(vec![], |mut all_chars, line| {
            ...
            'outer: for i in 0..half_num_of_char {
                let first_each_char = line.chars().nth(i as usize).unwrap();
...
fn get_total_sum(chars: Vec<char>) -> u32 {
    ...
    for i in chars {
        let temp = i as u32;
        if (65..=90).contains(&temp) {
            sum += temp - 38;
        } else if (97..=122).contains(&temp) {
            sum += temp - 96;
        }
    }
    sum
...
fn camp_cleanup(content: String) -> u32 {
    let sum = content.lines().into_iter().fold(0, |mut total, line| {
        ...
        let first_range: Vec<u32> = first_group
            .into_iter()
            .map(|a| a.trim().parse::<u32>().expect("Not a number"))
            .collect();
        let second_group = each_line[1].split('-').collect::<Vec<&str>>();
        let second_range: Vec<u32> = second_group
            .into_iter()
            .map(|a| a.trim().parse::<u32>().expect("Not a number"))
            .collect();
